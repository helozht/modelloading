...
const QString default_diffuse_path = ":/default_diffuse";
const QString default_specular_path = ":/default_specular";

Model::Model(QString path, QOpenGLContext* context, QOpenGLShaderProgram* shaderProgram, QMatrix4x4 model)
    : context(context)
    ...
    , directory(path)
{
    ...
    for(int i=0;i<4;i++){
        for(int j=0;j<4;j++){
            model_[i][j] = model(i, j);
        }
    }
    processNode(scene->mRootNode, scene, model_);
}
...
Model *Model::createModel(QString path, QOpenGLContext *context, QOpenGLShaderProgram *shaderProgram, QMatrix4x4 model)
{
    return new Model(path, context, shaderProgram, model);
}
...
Mesh* Model::processMesh(aiMesh *mesh,const aiScene *scene ,aiMatrix4x4 model)
{
    ...
    if (!diffuseMaps.size())
    {
        ...
        texture->texture.setData(QImage(default_diffuse_path));
        texture->type = "texture_specular";
        texture->path = default_diffuse_path;
    ...
    {
        Texture* texture = new Texture;
        texture->texture.setData(QImage(default_specular_path));
        texture->type = "texture_specular";
        texture->path = default_specular_path;
